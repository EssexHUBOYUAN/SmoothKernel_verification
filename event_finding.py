# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 15:34:26 2023
            ...
            if RACE:
                ...
                if line[1] == '0':
                    ...
                    for i in range(len(line)):
                        if i > 1:
                            try:
                                L = D_Block[N,float(line[1]),D_Race[i][0],D_Race[i][1]]
                                L.append(float(line[i]))
                                D_Block[N,float(line[1]),D_Race[i][0],D_Race[i][1]] = L
                            except KeyError:
                                D_Block[N,float(line[1]),D_Race[i][0],D_Race[i][1]] = [float(line[i])]
                        else:
                            pass
                else:
                    #print(line)
...
def output_corr(D_corr, D_market, typename):
    '''save a .csv file for corrilation coiff'''
    for country, product in D_market:
        filename = typename + '_corr_' + country +'_' + product + '.csv'
        Head = ['Block sample size','race','gender','age','slope','R2']
    ...
    for product in D_market:
        for array in D_Block:
            ...
            for age_line in D_Block[array]:
                ...
                D_corr_results[key] = fit_param, R2
...
def plot_3DBlock(Block_array,item, what=''):
    ...
    x = np.array([int(x) for x in range(85)])
    y = np.array([int(y) for y in range(85)])
    #print(type(x), type(y))
...
def for_each_country_main(country):
    for guide in L_guide:
        ...
        D_Block_exparray, D_col_para = get_expbyage(D_Block)
        ...
        D_market,L_year = read_market(marketfile)
        '''
        ...
        #print(D_Year)
        '''
        D_corr_Block = BlockVsTrend(D_Block, D_market, D_Year, L_year)
        print(len(D_corr_Block))
        output_corr(D_corr_Block, D_market, guide)
        del(D_corr_Block)
        '''
        D_corr_Block_ageexp = BlockVsTrend(D_Block_exparray, D_market, D_Year, L_year)
        output_corr(D_corr_Block_ageexp, D_market, guide)
        del(D_corr_Block_ageexp)
        
        #D_corr_Block_residue = BlockVsTrend(D_res_lam, D_market, D_Year, L_year)
    return


def main():
    for country in L_country:
        for_each_country_main(country)
    return
#main()
